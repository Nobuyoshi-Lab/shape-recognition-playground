...
from .utils.util_functions import capitalize_shape_name
...
model = load_model(
    os.path.abspath(
        os.path.join(
            ...
            'shape_recognition_model.h5'))
    )
shape_names = [
    capitalize_shape_name(shape) for shape in SHAPE_NAMES +
...
def recognize_shape():
    ...
    cv2.imwrite("preprocessed_image_debug.png",
                preprocessed_image)  # Save preprocessed image

    # Resizing the input image with cv2.INTER_AREA interpolation method
    gray_resized = cv2.resize(
        preprocessed_image, INPUT_SHAPE[:2], interpolation=cv2.INTER_AREA)

    # Save resized grayscale image
    cv2.imwrite("gray_resized_debug.png", gray_resized)
    ...
    if confidence < confidence_threshold:
        ...
        shape = capitalize_shape_name(shape)

    if shape == "Ice Cream":
        shape = "Bing Chilling"
    elif shape == "Triangle":
        shape = "Illuminati Confirmed"
